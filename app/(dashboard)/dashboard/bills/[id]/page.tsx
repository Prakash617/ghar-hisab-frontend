...
const initialPaymentHistory: Record<string, PaymentHistory[]> = {
    "101": [
...
// --- MAIN PAGE COMPONENT ---
export default function BillsDetailPage({params,}: {params: Promise<{ id: string }>}) 
{
  ...
  const [paymentHistory, setPaymentHistory] = useState<PaymentHistory[]>(initialPaymentHistory[roomId] || []);
  const [bills, setBills] = useState<Bill[]>(initialBills);

  const [previousUnits, setPreviousUnits] = useState(() => paymentHistory.length > 0 ? paymentHistory[paymentHistory.length - 1].currentUnits : 0);
  ...
  const saveEditedPayment = (index: number, data: Partial<PaymentHistory>) => {
    const updatedHistory = [...paymentHistory];
    const electricity = (data.currentUnits! - updatedHistory[index].previousUnits) * electricityRate;
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold">Room <span className="text-primary">{roomId}</span> Bill Details</h1>
      ...
      <ElectricityBillCard 
        ...
        saveEdited={() => {
            if (editingIndex !== null) {
                ...
                const lastReading = paymentHistory.length > 0 ? paymentHistory[paymentHistory.length - 1].currentUnits : 0;
        ...
        editingMonth={editingIndex !== null ? paymentHistory[editingIndex].month : undefined}
      />
